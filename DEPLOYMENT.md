...
Ask and Kindred are both in the patient app: `/?tab=ask` and `/?tab=kindred`. Ask uses the Anima ADK with the existing OpenAI key. When `DATABASE_URL` is set, Ask and patient switching query Neon directly; no Anima API key is needed. Anima settings are only a fallback for local environments without a database. `OPENAI_MODEL` optionally overrides Ask; by default both pages use `AGENT_MODEL` (`gpt-5.6-sol`).
