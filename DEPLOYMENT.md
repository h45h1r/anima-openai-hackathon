...
Ask and Kindred are both in the patient app: `/?tab=ask` and `/?tab=kindred`. Ask uses the Anima ADK with the existing OpenAI key. Its clinical source defaults to `SIM_BASE_URL` and `SIM_API_KEY`; optional `ANIMA_BASE_URL` and `ANIMA_API_KEY` override them. `OPENAI_MODEL` optionally overrides Ask; by default both pages use `AGENT_MODEL` (`gpt-5.6-sol`).
