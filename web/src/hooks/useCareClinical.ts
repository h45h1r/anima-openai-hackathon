...
import { careViewerForKindred, familyForLevelSync } from "@/lib/carecircle/viewers";

const SESSION_KEY = "kindred.careSessionId";
const DEFAULT_ANIMA_BASE = "https://sim.animahacks.com";
...
export function useCareClinical(kindred: AppState | null, kindredViewerId: string | null) {
      ...
      try {
        ...
        if (err instanceof ApiError && err.code === "patient_not_in_world") {
          ...
          const res = await careApi<{
            ...
            method: "POST",
            sessionId,
            body: JSON.stringify({ patientId: fallbackId }),
          });
          setSession(res.session);
          setPolicy(res.policy);
          setContext(res.context);
    ...
    async (state: AppState) => {
      ...
      try {
        ...
        const connected = await careApi<{ session: CareSession }>("/connect", {
          method: "POST",
          body: JSON.stringify({ baseUrl: DEFAULT_ANIMA_BASE }),
        });
  ...
  const ask = useCallback(
    async (question: string) => {
      if (!session?.selectedPatientId || !sid) throw new Error("Clinical Ask is not ready yet");
    ...
    [session, sid, askThread, suggestions],
  );
