...
| --- | --- | --- |
| 1. Sees it coming | In the app: the Home boxes (`web/src/components/patient/HealthOverview.tsx`, one tile per panel with a status word, tap for each measurement and its direction) and the Body tab (`web/src/components/body/BodyView.tsx`, `web/src/lib/body/systems.ts`: nine systems computed from the live record, consent-gated per system, licensed SOMA-X body and NIH Human Reference Atlas organs). Still prototype: the 30-day worsening rules and the six trend cards, `lib/insights.mjs` served by `server.mjs` at `GET /api/insights` | Kindred app for the views; prototype for the trend rules |
...
- **Demo patient.** `docs/demo-patients.md` recommends running the demo on Amira Khan (SIM-000001, 74, heart failure and CKD) because her record already holds a discharge, a letter promising a follow-up, an appointment today and a daughter in the notes. Eleanor is kept for the wearables and community story. The deck is written for Eleanor; if the team switches, slide 1, slide 4's example line and the demo steps need Amira's facts.
...
- Blind to the trend: Stow 2018 as above ("not effective at identifying individuals"). Eleanor's own record is the illustration: four low white cell counts, each filed as "available", none reviewed together.
