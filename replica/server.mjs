...
export const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (siteRoute) {
      ...
      return send(res, 200, { ...context.rows[0]?.body, resources: records.rows.map(r => r.body), resourceTotal: count.rows[0].total, resourceOffset: offset, resourceLimit: limit });
    }
...
const port = Number(process.env.REPLICA_PORT || 4180);
server.listen(port, '127.0.0.1', () => console.log(`Local replica API: http://localhost:${port} · database ${database}`));
